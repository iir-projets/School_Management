...
export default class LoginForm extends React.Component {
    ...
    onLogin = (e, username, password) => {
        e.preventDefault();
            const response =  axios.post('http://localhost:8099/login',{
                username: username,
                password: password
            }).then((response) => {this.props.onLoginSuccess()
    ...
    onRegister = (e, firstName,lastName,username, password) => {
        e.preventDefault();
        const response =  axios.post('http://localhost:8099/register',{
            username: username,
            password: password,
            firstName: firstName,
            lastName: lastName
        }).then((response) => {this.props.onLoginSuccess();
            this.props.history.push('/');}).catch((error) => {console.error('Error Login:', error)})
